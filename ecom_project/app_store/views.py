...
def store_view(request, category_slug=None):
    ...
    if category_slug != None:
        ...
        products_available = ProductClass.objects.all().filter(
...
def product_detail_view(request, category_slug, product_slug):
    try:
        single_product = ProductClass.objects.get(
            category__slug=category_slug, slug=product_slug)
        in_cart = CartItemClass.objects.filter(
...
def submit_review_view(request, product_id):
    ...
    if request.method == "POST":
        try:
            reviews = ReviewRatingClass.objects.get(
